"""
Optimisation 3: Frame skipping.

Change from base:
  - YOLO inference runs only every DETECT_EVERY frames
  - intermediate frames reuse the previous detection result
  - trades detection freshness for speed; acceptable when the cyclist moves
    slowly relative to frame rate

Encode is still CPU libx264 (same as base) so inference speedup is isolated.
No FP16 or batching — those are tested separately.
"""
...
CONFIDENCE   = 0.5
MODEL_PATH   = "best.pt"
DETECT_EVERY = 2   # run detection on every Nth frame; reuse result otherwise

if len(sys.argv) < 2:
    print("Usage: python3 infer_opt3_skip.py <video> [model.pt]")
    sys.exit(1)
...
output_path = base + "_opt3_skip.mp4"
temp_path   = base + "_opt3_skip_tmp.mp4"
...
print(f"Processing {frame_count} frames (detecting every {DETECT_EVERY}) → {output_path}")
t_start      = time.perf_counter()
frame_idx    = 0
cached_results = None   # last detection result, reused on skipped frames
...
while True:
    ...
    # --- OPT 3: only run model every DETECT_EVERY frames ---
    if frame_idx % DETECT_EVERY == 0:
        cached_results = model(frame, conf=CONFIDENCE, verbose=False)
