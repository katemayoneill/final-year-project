...
def get_xy(keypoints, idx):
    """Return integer (x, y) for keypoint idx if confidence >= CONF_MIN and position is valid, else None."""
...
def draw_skeleton(frame, keypoints):
    """Draw Body25 limb connections and joint dots onto frame in-place."""
...
def draw_angle_annotation(frame, keypoints, name_a, name_b, name_c, colour, prefix, arc_radius=45):
    ...
    A = get_xy(keypoints, JOINT[name_a]) if name_a in JOINT else None
    B = get_xy(keypoints, JOINT[name_b]) if name_b in JOINT else None
    C = get_xy(keypoints, JOINT[name_c]) if name_c in JOINT else None
    ...
    # Always draw the short arc (the measured angle, always ≤ 180°)
    ...
    # Short rays from B toward A and C to show which limbs form the angle
    ...
    # Label at arc midpoint, pushed outward
...
def build_graph_params(angle_series, peak_timestamps, cadence_rpm, knee_used, frame_w, frame_h):
    ...
    a_min = max(0.0,   min(angles) - 10)
    a_max = min(180.0, max(angles) + 10)
...
def draw_hud(frame, verdict, verdict_detail, cadence_rpm, frame_w, frame_h):
    """Draw verdict banner, detail line, and RPM label onto frame in-place."""
    font   = cv2.FONT_HERSHEY_SIMPLEX
    colour = VERDICT_COLOUR.get(verdict, (180, 180, 180))
    label  = verdict.replace("_", " ").upper()

    # Verdict banner — top centre
...
def main():
    """Parse arguments, annotate video with skeleton and verdict overlay, write final.mp4."""
    if len(sys.argv) < 5:
